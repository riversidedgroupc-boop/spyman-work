...
class TestBuildPRCurve:
    def test_one_perfect_prediction(self):
        gt_by_img = {"img.jpg": [_box(bbox=[0, 0, 100, 100])]}
        pred_by_img = {"img.jpg": [_box(bbox=[0, 0, 100, 100], conf=0.9)]}
        result = build_pr_curve(gt_by_img, pred_by_img, class_id=0, iou_threshold=0.5)
        assert result["ap"] == pytest.approx(1.0)

    def test_no_predictions(self):
        gt_by_img = {"img.jpg": [_box()]}
    ...
    def test_no_ground_truths(self):
        ...
        pred_by_img = {"img.jpg": [_box()]}
        result = build_pr_curve(gt_by_img, pred_by_img, class_id=0)
        assert result["ap"] == 0.0

    def test_all_wrong(self):
        gt_by_img = {"img.jpg": [_box(bbox=[0, 0, 10, 10])]}
        pred_by_img = {"img.jpg": [_box(bbox=[90, 90, 100, 100], conf=0.9)]}
        result = build_pr_curve(gt_by_img, pred_by_img, class_id=0)
        assert result["ap"] == 0.0
    ...
    def test_mixed_tp_fp(self):
        ...
        pred_by_img = {
            "img.jpg": [
                _box(bbox=[0, 0, 100, 100], conf=0.9),  # TP
                _box(bbox=[200, 200, 300, 300], conf=0.8),  # FP
            ]
        }
        result = build_pr_curve(gt_by_img, pred_by_img, class_id=0)
    ...
    def test_pr_curve_non_increasing_precision(self):
        ...
        pred_by_img = {
            "img.jpg": [
                _box(bbox=[0, 0, 100, 100], conf=0.9),
                _box(bbox=[200, 200, 300, 300], conf=0.8),
            ]
        }
        result = build_pr_curve(gt_by_img, pred_by_img, class_id=0)
...
class TestComputeMAP:
    def test_single_class_perfect(self):
        gt_by_img = {"img.jpg": [_box(cls_id=0, bbox=[0, 0, 100, 100])]}
        pred_by_img = {"img.jpg": [_box(cls_id=0, bbox=[0, 0, 100, 100], conf=0.9)]}
    ...
    def test_multiple_classes(self):
        gt_by_img = {
            "img.jpg": [
                _box(cls_id=0, cls_name="scratch", bbox=[0, 0, 100, 100]),
                _box(cls_id=1, cls_name="dent", bbox=[50, 50, 150, 150]),
            ]
        }
        pred_by_img = {
            "img.jpg": [
                _box(cls_id=0, cls_name="scratch", bbox=[0, 0, 100, 100], conf=0.9),
                _box(cls_id=1, cls_name="dent", bbox=[50, 50, 150, 150], conf=0.8),
            ]
        }
