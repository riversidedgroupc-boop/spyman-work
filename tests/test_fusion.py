...
class TestFusePredictionsUnion:
    def test_basic(self):
        a = _preds_dict([make_detection_box(bbox=[0, 0, 100, 100])])
        b = _preds_dict([make_detection_box(bbox=[90, 90, 150, 150])])
        result = fuse_predictions_union(a, b, iou_threshold=0.5)
        assert len(result["img.jpg"]) >= 1

    def test_non_overlapping(self):
        a = _preds_dict([make_detection_box(bbox=[0, 0, 10, 10])])
        b = _preds_dict([make_detection_box(bbox=[50, 50, 60, 60])])
        result = fuse_predictions_union(a, b, iou_threshold=0.5)
        assert len(result["img.jpg"]) == 2

    def test_different_images(self):
        a = _preds_dict([make_detection_box(img="img1.jpg")])
        b = _preds_dict([make_detection_box(img="img2.jpg")])
        result = fuse_predictions_union(a, b, iou_threshold=0.5)
...
class TestFusePredictionsIntersection:
    def test_overlapping(self):
        a = _preds_dict([make_detection_box(bbox=[0, 0, 100, 100], conf=0.9)])
        b = _preds_dict([make_detection_box(bbox=[10, 10, 90, 90], conf=0.8)])
...
class TestMarkReviewCandidates:
    def test_basic(self):
        primary = _preds_dict([make_detection_box(bbox=[0, 0, 100, 100])])
        secondary = _preds_dict([make_detection_box(bbox=[90, 90, 150, 150])])
        result = mark_review_candidates(primary, secondary, iou_threshold=0.5)
        assert len(result["img.jpg"]) >= 1

    def test_secondary_only(self):
        primary = _preds_dict([])
        secondary = _preds_dict([make_detection_box(bbox=[50, 50, 60, 60])])
        result = mark_review_candidates(primary, secondary, iou_threshold=0.5)
        assert len(result["img.jpg"]) == 1
...
class TestFuseWeightedMerge:
    def test_basic(self):
        a = _preds_dict([make_detection_box(bbox=[0, 0, 100, 100], conf=0.9)])
        b = _preds_dict([make_detection_box(bbox=[10, 10, 90, 90], conf=0.7)])
...
class TestEvaluateFusionStrategy:
    def test_union(self):
        ...
        preds = [
            _preds_dict([make_detection_box(bbox=[10, 10, 90, 90])]),
            _preds_dict([make_detection_box(bbox=[50, 50, 60, 60])]),
        ]
