...
class TestMatchDetections:
    def test_one_match(self):
        gt = [make_detection_box(bbox=[0, 0, 100, 100])]
        pred = [make_detection_box(bbox=[10, 10, 90, 90])]
    ...
    def test_duplicate_predictions(self):
        gt = [make_detection_box(bbox=[0, 0, 100, 100])]
        pred = [
            make_detection_box(bbox=[10, 10, 90, 90], conf=0.9),
            make_detection_box(bbox=[20, 20, 80, 80], conf=0.8),
        ]
        result = match_detections(gt, pred, iou_threshold=0.5)
        assert len(result["matches"]) == 1
    ...
    def test_class_mismatch_class_aware(self):
        gt = [make_detection_box(cls_id=0, cls_name="scratch", bbox=[0, 0, 100, 100])]
        pred = [make_detection_box(cls_id=1, cls_name="dent", bbox=[10, 10, 90, 90])]
    ...
    def test_low_iou_no_match(self):
        gt = [make_detection_box(bbox=[0, 0, 10, 10])]
        pred = [make_detection_box(bbox=[90, 90, 100, 100])]
        result = match_detections(gt, pred, iou_threshold=0.5)
        assert len(result["matches"]) == 0
        assert len(result["false_positives"]) == 1
        assert len(result["false_negatives"]) == 1
    ...
    def test_prediction_matches_best_gt(self):
        gt = [
            make_detection_box(bbox=[0, 0, 100, 100], cls_id=0),
            make_detection_box(bbox=[50, 50, 150, 150], cls_id=0),
        ]
        pred = [make_detection_box(bbox=[40, 40, 110, 110], cls_id=0)]
