...
class EvaluationPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self._img_label.setStyleSheet("color: #888;")
        ...
        self._lbl_label.setStyleSheet("color: #888;")
        ...
        metrics_w = QWidget(); ml = QVBoxLayout(metrics_w)
        self._map50_label = QLabel()
        bind(self._map50_label, "eval.map50_na")
        self._map50_label.setStyleSheet("font-size: 20px; font-weight: bold; color: #4CAF50;")
        ml.addWidget(self._map50_label)
        self._map50_95_label = QLabel()
        bind(self._map50_95_label, "eval.map50_95", value="—")
        self._map50_95_label.setStyleSheet("font-size: 16px; color: #81C784;")
        ...
        self._per_class_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        ...
        self._summary_label.setStyleSheet("color: #888; font-size: 14px;")
    ...
    def _browse_dir(self, kind):
        dir_path = QFileDialog.getExistingDirectory(self, f"选择{'图片' if kind == 'img' else '标签'}目录")
        ...
        if kind == "img":
            ...
            self._lbl_label.setText(dir_path); self._label_dir = dir_path

    def _run_evaluation(self):
        model_id = self._model_combo.currentData()
        if not model_id: QMessageBox.information(self, tr("app.tip"), tr("app.select_model")); return
        if not self._image_paths: QMessageBox.information(self, tr("app.tip"), tr("app.select_project")); return
        ...
        self._worker = InferenceWorker(model_path=model.model_path, image_paths=self._image_paths, model_type=model.model_type, confidence=0.01)
        ...
        self._progress.setVisible(True); self._progress.setMaximum(len(self._image_paths))
        self._worker.start()
    ...
    def _on_eval_finished(self):
                ...
                with open(os.path.join(self._label_dir, fname)) as f:
                    for line in f:
                        ...
                        if len(parts) >= 5:
                            ...
                            gts.append(DetectionBox(image_name=img_name, class_id=cls_id, class_name=str(cls_id), confidence=1.0, bbox=[xc - w/2, yc - h/2, xc + w/2, yc + h/2]))
        ...
        if gts_by_image:
            try:
                ...
                for gts in gts_by_image.values():
                    for gt in gts: class_ids.add(gt.class_id)
                if not class_ids: class_ids = {0}
                map_result = compute_map(all_gts, all_preds, list(class_ids), [0.5, 0.75])
                if 'mAP@0.5' in map_result:
                    bind(self._map50_label, "eval.map50", value=f"{map_result.get('mAP@0.5', 0):.4f}")
                else:
                    bind(self._map50_label, "eval.map50_na")
                self._map50_95_label.setText(tr("eval.map50_95", value=f"{map_result.get('mAP@0.5:0.95', 0):.4f}") if 'mAP@0.5:0.95' in map_result else "")
                # Per-class
        ...
        self._summary_label.setText(tr("eval.completed", images=len(preds), gt_images=len(gts_by_image), boxes=total_pred_boxes))

    def _on_error(self, err):
        self._progress.setVisible(False); QMessageBox.critical(self, tr("eval.error_title"), err)
