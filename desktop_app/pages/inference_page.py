...
class InferencePage(QWidget):
    ...
    def _build_ui(self):
        ...
        top.addWidget(run_btn); top.addStretch()
        ...
        self._viewer = DefectOverlayView(); splitter.addWidget(self._viewer)
        ...
        nav.addWidget(self._pos_label); nav.addStretch()
        right_layout.addLayout(nav)

        self._det_table = QTableWidget(0, 4)
        self._det_table.setHorizontalHeaderLabels([tr("inference.col_class"), tr("inference.col_conf"), tr("inference.col_bbox"), tr("inference.col_area")])
        ...
        splitter.addWidget(right); splitter.setSizes([600, 400])
    ...
    def showEvent(self, event): super().showEvent(event); self._refresh_models()
    ...
    def _browse_dir(self):
        ...
        self._image_paths = sorted([os.path.join(dir_path, f) for f in os.listdir(dir_path) if os.path.splitext(f)[1].lower() in exts])
        self._predictions.clear(); bind(self._pos_label, "inference.position", current=0, total=len(self._image_paths))
        if self._image_paths:
            self._current_index = 0; self._viewer.load_image(self._image_paths[0]); self._viewer.clear_detections()

    def _run_inference(self):
        model_id = self._model_combo.currentData()
        if not model_id: QMessageBox.information(self, tr("app.tip"), tr("app.select_model")); return
        if not self._image_paths: QMessageBox.information(self, tr("app.tip"), tr("inference.select_image_dir_first")); return
        model = get_model_version(model_id)
        if not model or not model.model_path: QMessageBox.warning(self, tr("app.error"), tr("inference.model_path_empty")); return

        self._worker = InferenceWorker(model_path=model.model_path, image_paths=self._image_paths, model_type=model.model_type, confidence=0.25)
        self._worker.progress.connect(lambda c, t: self._progress.setValue(c))
        self._worker.result_ready.connect(self._on_result)
        self._worker.finished.connect(self._on_finished)
        self._worker.error.connect(self._on_error)
        self._predictions.clear(); self._progress.setVisible(True); self._progress.setMaximum(len(self._image_paths))
        self._worker.start()
    ...
    def _on_finished(self):
        ...
        if self._image_paths and self._current_index >= 0: self._show_current()

    def _on_error(self, err):
        self._progress.setVisible(False); QMessageBox.critical(self, tr("inference.inference_error"), err)

    def _navigate(self, delta):
        if not self._image_paths: return
        self._current_index = (self._current_index + delta) % len(self._image_paths); self._show_current()

    def _show_current(self):
        if self._current_index < 0 or self._current_index >= len(self._image_paths): return
        path = self._image_paths[self._current_index]; self._viewer.load_image(path)
        bind(self._pos_label, "inference.position", current=self._current_index + 1, total=len(self._image_paths))
        pred = self._predictions.get(path)
        if pred: self._show_prediction(pred)
        else: self._viewer.clear_detections(); self._det_table.setRowCount(0)

    def _show_prediction(self, prediction):
        dets = prediction.detections if hasattr(prediction, 'detections') else []
        self._viewer.set_detections(dets); self._det_table.setRowCount(len(dets))
        for row, det in enumerate(dets):
            ...
            self._det_table.setItem(row, 2, QTableWidgetItem(f"[{bbox[0]:.0f},{bbox[1]:.0f},{bbox[2]:.0f},{bbox[3]:.0f}]"))
