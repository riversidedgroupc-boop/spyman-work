...
class ModelVersionPage(QWidget):
    ...
    def _build_ui(self):
        ...
        activate_btn.clicked.connect(self._activate_model)
        ...
        status_layout.addWidget(set_btn)
    ...
    def _refresh(self):
        ...
        for row, m in enumerate(models):
            ...
            active_item = QTableWidgetItem(
                tr("model.active") if m.is_active else tr("model.inactive")
            )
            if m.is_active:
    ...
    def _delete_model(self):
        ...
        if (
            QMessageBox.question(self, tr("app.confirm_delete"), tr("model.delete_confirm", id=mid))
            == QMessageBox.StandardButton.Yes
        ):
            delete_model_version(mid)
            self._refresh()
    ...
    def _set_status(self):
        ...
        if row < 0:
            return
        mid = self._table.item(row, 0).text()
        update_model_version(mid, status=self._status_combo.currentData())
        self._refresh()
    ...
    def _activate_model(self):
        ...
        mid = self._table.item(row, 0).text()
        model = (
            get_active_model(self._ctx.current_project_id) if self._ctx.current_project_id else None
        )
        ...
        if (
            QMessageBox.question(self, tr("model.activate_btn"), msg)
            == QMessageBox.StandardButton.Yes
        ):
            result = activate_model(mid)
            if result:
                QMessageBox.information(
                    self, tr("app.completed"), tr("model.activated", name=result.model_name)
                )
                self._refresh()
                self.data_changed.emit()
    ...
    def _rollback_model(self):
        ...
        if (
            QMessageBox.question(
                self, tr("model.rollback_btn"), tr("model.rollback_confirm", name=mid)
            )
            == QMessageBox.StandardButton.Yes
        ):
            ...
            if result:
                QMessageBox.information(
                    self, tr("app.completed"), tr("model.rolled_back", name=result.model_name)
                )
                self._refresh()
                self.data_changed.emit()
...
class RegisterModelDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        form.addRow(model_name_label, self._name_edit)
        ...
        self._path_edit.setPlaceholderText(tr("model.model_path_placeholder"))
        ...
        self._base_edit.setPlaceholderText(tr("model.base_placeholder"))
        form.addRow(base_model_label, self._base_edit)
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, tr("dialog.select_model_file"), "", "Models (*.pt *.onnx);;All (*)"
        )
        if path:
    ...
    def get_data(self):
        return {
            "model_name": self._name_edit.text().strip() or tr("app.unsaved"),
            "model_type": self._type_combo.currentData(),
            "model_path": self._path_edit.text().strip(),
            "base_model": self._base_edit.text().strip() or None,
        }
