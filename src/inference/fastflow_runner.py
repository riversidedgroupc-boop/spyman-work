"""FastFlow anomaly-detection runner.
...
class FastFlowRunner(BaseRunner):
    """FastFlow anomaly runner.
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        super().__init__("fastflow", config)
    ...
    def load_model(self) -> None:
        ...
        if self.mode == "real":
            ...
            _log.info("FastFlow mock mode enabled")
        else:
            _log.warning(
                "Unknown FastFlow mode '%s', falling back to mock", self.mode
            )
            self.mode = "mock"
            self._is_loaded = True

    def _load_real_model(self) -> None:
        """Attempt to load the FastFlow model via anomalib."""
        try:
            import anomalib  # noqa: F401
        except ImportError:
            _log.warning(
                "anomalib not installed. FastFlow real mode requires anomalib. "
        ...
        if model_file and not model_file.exists():
            _log.warning(
                "FastFlow model file not found: %s. Falling back to mock mode.",
                model_file,
            )
            self.mode = "mock"
            self._is_loaded = True
        ...
        try:
            ...
            _log.info("anomalib FastFlow available (model loading deferred)")
        except Exception:
            _log.warning(
                "Failed to import anomalib FastFlow. Falling back to mock mode."
            )
            self.mode = "mock"
    ...
    def _load_imported_results(self) -> None:
        ...
        if not result_path.exists():
            _log.warning(
                "FastFlow result file not found: %s. Using mock mode.",
                self.result_file,
            )
            self.mode = "mock"
            self._is_loaded = True
        ...
        _log.info("FastFlow imported %d results", len(self._imported_results))
        self._is_loaded = True

    # ------------------------------------------------------------------ predict

    def predict(self, image_path: str | Path) -> UnifiedPrediction:
        """Run FastFlow inference (or mock / import)."""
        ...
        return UnifiedPrediction(
            ...
            runtime_ms=elapsed,
            extra={"warning": "Real FastFlow inference not yet implemented"},
        )
